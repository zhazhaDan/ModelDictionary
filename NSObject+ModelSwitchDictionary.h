//
//  NSObject+ModelSwitchDictionary.h
//  GDCommonDemo
//
 ...
 *  @param dict 要转model的字典
 */
- (instancetype) setDeepModelPropertyForObj:(NSObject *)obj isCleanValue:(BOOL)isClean;//有问题，要修改
- (instancetype) setDeepModelPropertyForObj:(NSObject *)obj avoidKeys:(NSArray *)avoidKeys isCleanValue:(BOOL)isClean;

@end


@interface NSDictionary(SpecialKey)

- (id)valueForSpecialKey:(NSString *)key;

@end

@interface NSObject(ValueKey)
